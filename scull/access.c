...
static int scull_u_open(struct inode *inode, struct file *filp)
{
	...
	if (scull_u_count && 
			(scull_u_owner != __kuid_val(current->cred->uid)) &&  /* allow user */
			(scull_u_owner != __kuid_val(current->cred->euid)) && /* allow whoever did su */
	...
	if (scull_u_count == 0)
		scull_u_owner = __kuid_val(current->cred->uid); /* grab it */
...
static inline int scull_w_available(void)
{
	return scull_w_count == 0 ||
		scull_w_owner == __kuid_val(current->cred->uid) ||
		scull_w_owner == __kuid_val(current->cred->euid) ||
...
static int scull_w_open(struct inode *inode, struct file *filp)
{
	...
	if (scull_w_count == 0)
		scull_w_owner = __kuid_val(current->cred->uid); /* grab it */
	scull_w_count++;
	spin_unlock(&scull_w_lock);
